        ...
        try {
            ...
            if (instrumentationActive) {
                try {
                    ...
                    AgentLogger.error("Instrumentation initialization failed; proceeding without instrumentation. Reason: " + t.getMessage());
                    traceFileLogger.error("Instrumentation initialization failed; proceeding without instrumentation");
                }
            } else {
            ...
            if (AgentLogger.isInitialized()) {
                AgentLogger.error(String.format("Exiting %s Java Agent due to exception - %s", AGENT_NAME, t.getMessage()));
            } else {
                System.err.println("Failure in agent startup: " + t.getMessage());
            }
        }
    }
